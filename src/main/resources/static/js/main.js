...
$.ajax({
    ...
    type: 'GET',
    dataType: 'json',
    success: function(loadHighScore) {
    	document.getElementById("highscore").innerHTML = loadHighScore;
    	highscore=loadHighScore;
    	console.log('i ran'+loadHighScore)
    }
	...
	function checkThrow(x, y) {
		...
		if (score > 0) {
			...
			totalScore += score;			
			var a=document.getElementById("targetScore").innerHTML;
			document.getElementById("left").innerHTML=(a-totalScore);
	...
	function end() {
		...
		if(score===target){
			...
			if(streak>highscore){
				highscore=streak;
				$.ajax({
					    url: '/updateHighScore?score='+highscore,
					    type: 'GET',
					    dataType: 'json',
					    success: function(loadHighScore) {
					    	console.log('i ran 2'+loadHighScore)
					    }
			...
			var out = outShot[Math.floor(Math.random() * outShot.length)];
			document.getElementById("targetScore").innerHTML = out ;
